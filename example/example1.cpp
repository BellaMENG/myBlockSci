...
void createIndexList(string fp, int max_block=600000) {
    Blockchain chain(fp);
    int64_t block_id = 1;
    int64_t index = 0;
...
void generateIndexListNEdgeList(string fp, int max_block=600000) {
    flat_hash_map<Address, int64_t> addr_id;
    vector<int64_t> edge_list;
    ...
    ofstream myfile;
    string edgeListfn = "edge_list";
    string indexListfn = "index_list";
    edgeListfn += to_string(max_block);
    indexListfn += to_string(max_block);
    edgeListfn += ".txt";
    indexListfn += ".txt";
    myfile.open(edgeListfn);
    for (int i = 0; i < edge_list.size() - 1; i += 2) {
        myfile << edge_list[i] << " " << edge_list[i+1] << "\n";
    }
    myfile.close();
    myfile.open(indexListfn);
    vector<pair<int64_t, string>>::iterator vec_itr;
    for (vec_itr = idxpairs.begin(); vec_itr != idxpairs.end(); ++vec_itr) {
        myfile << (*vec_itr).first << " " << (*vec_itr).second << "\n";
    }
...
void generateEdgeList(string fp, int max_block=600000) {
    ...
    for (auto block: chain) {
        ...
        if (block_id % 10000 == 0) {
            ...
            cout << "size of edge_list: " << edge_list.size() << endl;
        }
    ...
    fn += ".txt";
    cout << "the size of edge_list vector is: " << edge_list.size() << endl;
    cout << "the max_size of edge_list vector is: " << edge_list.max_size() << endl;

    myfile.open(fn);
    for (int i = 0; i < edge_list.size() - 1; i += 2) {
        myfile << edge_list[i] << " " << edge_list[i+1] << "\n";
    }
    cout << "last element of edge_list: " << edge_list[edge_list.size()-1] << endl;
    myfile.close();
}
...
//    generateIndexListNEdgeList(block_fp, 400000);
//    createIndexList(block_fp, 200000);
    generateEdgeList(block_fp, 400000);
